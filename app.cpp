...
# endif /* USE_CONSOLEKIT */
		...
		if (session != "default") {
			Cfg::replaceVariables(loginCommand, SESSION_VAR, session);
			Cfg::replaceVariables(loginCommand, THEME_VAR, themeName);
			string sessStart = cfg->getOption("sessionstart_cmd");
			if (sessStart != "") {
				Cfg::replaceVariables(sessStart, USER_VAR, pw->pw_name);
				system(sessStart.c_str());
			}
			Su.Login(loginCommand.c_str(), mcookie.c_str(), session);
		} else {
			Su.DefaultLogin(loginCommand, mcookie.c_str());
		}
		_exit(OK_EXIT);
	}
	...
	if (WIFEXITED(status) && WEXITSTATUS(status)) {
		...
		sleep(3);
		 ...
		 if (sessStop != "") {
			Cfg::replaceVariables(sessStop, USER_VAR, pw->pw_name);
